<template>
  <div class="bg-slate-800/50 border border-slate-700 rounded-xl overflow-hidden flex flex-col max-h-[80vh]">
    ...
    <div class="flex items-center justify-between px-5 py-4 border-b border-slate-700 shrink-0">
      ...
      <button @click="load" :disabled="loading" class="p-1.5 text-slate-400 hover:text-white hover:bg-slate-700 rounded-lg transition-colors" title="Recargar">
      ...
      <div class="space-y-1.5 overflow-y-auto flex-1 min-h-0 pr-1">
        ...
        <div v-for="entry in store.blacklist" :key="entry.ip"
          ...
          <button @click="remove(entry.ip)" :disabled="removing === entry.ip"
            ...
            <div v-if="removing === entry.ip" class="w-3 h-3 border border-slate-400 border-t-transparent rounded-full animate-spin" />
...
async function load() {
  loading.value = true
  try { await store.fetchBlacklist() }
  catch { toast.error('No se pudo recargar la lista negra', { duration: 5000 }) }
...
async function add() {
  ...
  if (store.whitelist.some(e => e.ip === val)) {
    toast.error(`${val} ya está en la lista blanca`, { description: 'Eliminala de la lista blanca antes de bloquearla.', duration: 6000 })
    return
  }
  adding.value = true
  try {
    const body = await store.addToBlacklist(val, mot)
    if (body.ok) { toast.success(`${val} bloqueada`, { description: mot, duration: 4000 }); ip.value = ''; motivo.value = '' }
    else if (body.error) toast.error('Motor Prolog no disponible', { description: 'Reiniciá el servidor con: swipl server.pl', duration: 7000 })
    else toast.warning(String(body.message ?? 'La IP ya está en la lista'), { duration: 5000 })
  } catch { toast.error('Error al contactar el motor Prolog', { duration: 5000 }) }
...
async function remove(val: string) {
  ...
  try {
    ...
    if (body.ok) toast.success(`${val} removida de la lista negra`, { duration: 4000 })
    else toast.warning(String(body.message ?? 'No se pudo remover'), { duration: 5000 })
  } catch { toast.error('Error al contactar el motor Prolog', { duration: 5000 }) }
