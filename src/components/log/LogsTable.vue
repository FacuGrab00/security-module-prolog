...
function rowClass(log: LogEntry)         { return log.result === 'failure' ? 'bg-red-500/5' : ''; }
function ipClass(ip: string)             { return props.isBlocked(ip) ? 'text-red-400' : 'text-slate-300'; }
function resultBadgeClass(log: LogEntry) { return log.result === 'success' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-red-500/20 text-red-400'; }
function resultLabel(log: LogEntry)      { return log.result === 'success' ? 'Éxito' : 'Fallo'; }
function countryFlag(country: string)    { return FLAGS[country] ?? '🌐'; }
